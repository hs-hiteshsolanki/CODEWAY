...
import com.codeway_task.to_do_list.activity.MainActivity;
import com.codeway_task.to_do_list.activity.TaskCompletedActivity;
...
//    }
    public void showPopUpMenu(View view, int position) {
        ...
        popupMenu.setOnMenuItemClickListener(item -> {
            int itemId = item.getItemId();
            if (itemId == R.id.menuDelete) {
                // Check if AlertDialog is already showing
                if (!((Activity) context).isFinishing()) {
                    ...
                    alertDialogBuilder.setTitle(R.string.delete_confirmation).setMessage(R.string.sureToDelete)
                            ...
                            .setCancelable(false); // Prevent dialog from being dismissed on outside touch or back button
                ...
                AlertDialog.Builder completeAlertDialog = new AlertDialog.Builder(context, R.style.AppTheme_Dialog);
                completeAlertDialog.setTitle(R.string.confirmation).setMessage(R.string.sureToMarkAsComplete)
                        .setPositiveButton(android.R.string.ok, (dialog, which) -> showCompleteDialog(task.getTaskId(), position))
                        .setNegativeButton(android.R.string.cancel, (dialog, which) -> dialog.dismiss())
                        .setCancelable(false) // Prevent dialog from being dismissed on outside touch or back button
                        .show();
            }
...
//    }
    public void showCompleteDialog(int taskId, int position) {
        ...
        Button close = dialog.findViewById(R.id.closeButton);
        close.setOnClickListener(view -> {
            // Start TaskCompletedActivity
            Intent intent = new Intent(context, TaskCompletedActivity.class);
            context.startActivity(intent);

            // Delete the task from the database and refresh the UI
            deleteTaskFromId(taskId, position);
            dialog.dismiss();
        });
        dialog.getWindow().setBackgroundDrawable(new ColorDrawable(android.graphics.Color.TRANSPARENT));
